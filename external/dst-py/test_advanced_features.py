"""
Tests for advanced contextual discounting and combination rules.

This module tests the implementation of:
- Contextual discounting methods
...
from dempster_shafer.discounting.contextual.contextual import (
    classical_discount,
    contextual_discount,
...
class TestContextualDiscounting(unittest.TestCase):
    ...
    def test_classical_discount(self):
        """Test classical discounting."""
        # Test with alpha = 0 (no discounting)
        m_disc_0 = classical_discount(self.m, 0)
        self.assertEqual(m_disc_0, self.m)
        
        # Test with alpha = 1 (complete discounting)
        m_disc_1 = classical_discount(self.m, 1)
        self.assertEqual(m_disc_1, MassFunction({('a', 'b', 'c'): 1.0}, self.frame))
        
        # Test with alpha = 0.5 (partial discounting)
        m_disc_05 = classical_discount(self.m, 0.5)
        expected = {
            ('a',): 0.1,
            ('b',): 0.15,
            ('a', 'b'): 0.05,
            ('a', 'b', 'c'): 0.7  # 0.4 * 0.5 + 0.5
        }
        for focal_set, mass in expected.items():
            self.assertAlmostEqual(m_disc_05[focal_set], mass)
            
        # Test with invalid alpha
        with self.assertRaises(ValueError):
            classical_discount(self.m, -0.1)
        with self.assertRaises(ValueError):
            classical_discount(self.m, 1.1)
    ...
    def test_compute_generalization_matrix(self):
        ...
        # G(A,A) should be the product of all alphas
        self.assertAlmostEqual(G.get((A, A), 0), 0.2 * 0.3 * 0.4)
        
        # G(A,B) should be the product of alphas for elements in A∩B and betas for elements in B̄
        self.assertAlmostEqual(G.get((A, B), 0), 0.2 * 0.3 * 0.6)  # 0.6 = 1 - 0.4 (beta for 'c')
...
class TestAdvancedCombinationRules(unittest.TestCase):
    ...
    def test_canonical_conjunctive_decomposition(self):
        ...
        # Test with dogmatic mass function (should raise ValueError)
        m_dogmatic = MassFunction({
            ('a',): 0.2,
            ('b',): 0.3,
            (): 0.1,  # Empty set (contradiction)
            ('a', 'b', 'c'): 0.4
        }, self.frame)
        
        with self.assertRaises(ValueError):
            canonical_conjunctive_decomposition(m_dogmatic)
