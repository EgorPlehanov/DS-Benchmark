...
def write_reports(base_dir: Path, timestamp: str, report_json: dict[str, Any], report_txt: str) -> tuple[Path, Path]:
    """Сохраняет отчеты в processed_results/<report_type>/<timestamp>/..."""
    processed_root = base_dir / "processed_results"

    json_dir = processed_root / "comparison_json" / timestamp
    txt_dir = processed_root / "comparison_table" / timestamp

    json_dir.mkdir(parents=True, exist_ok=True)
    txt_dir.mkdir(parents=True, exist_ok=True)

    json_path = json_dir / "comparison_report.json"
    txt_path = txt_dir / "comparison_report.txt"
